...
当前地图采用分级渲染策略：全球/区域级保持 3D 球体；缩放到 14 级后自动把镜头切换为约 58° 鸟瞰角，并使用 CARTO `building` 矢量图层的 `render_height` / `render_min_height` 字段生成建筑白模。建筑在 14～15.5 级之间渐进拉伸，避免突然跳变；缩回 13 级以下恢复俯视。

```mermaid
flowchart LR
  A["全球级 z<13\n3D 球体 + 新闻态势"] -->|"持续放大"| B["城市过渡 z=13~14\n自动调整 pitch"]
  B --> C["城市级 z>=14\n建筑 fill-extrusion 白模"]
  C -->|"后续接入"| D["DEM / DSM\n地形与地表高程"]
  D --> E["遥感正射影像\nCOG / WMTS / XYZ"]
  E --> F["实景三维模型\n3D Tiles / glTF / GLB"]
  F --> G["地理实体与情报事件\n空间关联、查询、分析"]
