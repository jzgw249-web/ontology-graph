...
| 阶段 | 成果 | 页面 |
|---|---|---|
| 共现图谱 | 约 6760 个实体、26647 条边，公理 C1 自动纠正国家类型 | `graph.html` |
...
地图应正常显示左侧新闻列表；同一地点只显示一个带事件数量的标识；点击标识出现可滚动新闻列表；点击新闻打开原文。地图采用 MapLibre 3D 球体，底图地名为原始英文（曾试过换成中文，因底图数据不区分简繁体、无法保证只出简体而回滚），高缩放级别显示矢量建筑拉伸层。第三方实景 3D Tiles 试验已撤回，当前无相关密钥依赖。
